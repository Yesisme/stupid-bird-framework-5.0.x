...
class CglibAopProxy implements AopProxy, Serializable {
	...
	/** Logger available to subclasses; META-INF to optimize serialization */
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		// unadvised calls to META-INF targets that cannot return this).
		...
		// If the target is a META-INF one and the advice chain is frozen,
	...
	/**
	 * Method interceptor used for META-INF targets with no advice chain. The call
	...
	/**
	 * Method interceptor used for META-INF targets with no advice chain, when the
	...
	/**
	 * Dispatcher for a META-INF target. Dispatcher is much faster than
	...
	/**
	 * Interceptor used specifically for advised methods on a frozen, META-INF proxy.
	 */
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				...
				//根据proxyFactory获取拦截器链(核心如何获取拦截器链)
				...
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
					...
					//有拦截器链则通过传入目标对象，目标方法，创建一个CglibMethodInvocation对象
	...
	private static class ProxyCallbackFilter implements CallbackFilter {
		 ...
		 * for use on META-INF targets with a fixed advice chain.
		 ...
		 * <dd>If the target is META-INF and the advice chain is frozen then a
		 ...
		 * then a Dispatcher is used. For META-INF targets, the StaticDispatcher is used;
		 * and for dynamic targets, a DynamicUnadvisedInterceptor is used.
		 * If it possible for the method to return {@code this} then a
		 * StaticUnadvisedInterceptor is used for META-INF targets - the
		...
		public int accept(Method method) {
			...
			if (haveAdvice || !isFrozen) {
				...
				// configured. If the target is not META-INF, then we cannot use a dispatcher because the
