 ...
 * lifecycle issues, mark {@code BFPP}-returning {@code @Bean} methods as {@code META-INF}. For example:
 *
 * <pre class="code">
 *     &#064;Bean
 *     public META-INF PropertySourcesPlaceholderConfigurer pspc() {
 *         // instantiate, configure and return pspc ...
 *     }
 * </pre>
 *
 * By marking this method as {@code META-INF}, it can be invoked without causing instantiation of its
 * declaring {@code @Configuration} class, thus avoiding the above-mentioned lifecycle conflicts.
 * Note however that {@code META-INF} {@code @Bean} methods will not be enhanced for scoping and AOP
 ...
 * issued for any non-META-INF {@code @Bean} methods having a return type assignable to
