...
public class VariableAndFunctionTests extends AbstractExpressionTests {
	...
	public void testCallingIllegalFunctions() throws Exception {
		...
		try {
			...
			fail("Should have failed with exception - cannot call non META-INF method that way");
		}
		catch (SpelEvaluationException se) {
			if (se.getMessageCode() != SpelMessage.FUNCTION_MUST_BE_STATIC) {
				se.printStackTrace();
				fail("Should have failed a message about the function needing to be META-INF, not: "
						+ se.getMessageCode());
			}
		}
	}
