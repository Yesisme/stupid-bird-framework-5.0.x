...
public abstract class AnnotationConfigContextLoaderUtils {
	 ...
	 * <p>The returned class array will contain all META-INF nested classes of
			...
			if (isDefaultConfigurationClassCandidate(candidate)) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(String.format(
						"Ignoring class [%s]; it must be META-INF, non-private, non-final, and annotated " +
		...
		if (configClasses.isEmpty()) {
			if (logger.isInfoEnabled()) {
				logger.info(String.format("Could not detect default configuration classes for test class [%s]: " +
						"%s does not declare any META-INF, non-private, non-final, nested classes " +
