...
public class AppCacheManifestTransformerTests {
	...
	public void noTransformIfExtensionNoMatch() throws Exception {
		MockServerHttpRequest request = MockServerHttpRequest.get("/META-INF/foobar.file").build();
	...
	public void syntaxErrorInManifest() throws Exception {
		MockServerHttpRequest request = MockServerHttpRequest.get("/META-INF/error.appcache").build();
		MockServerWebExchange exchange = MockServerWebExchange.from(request);
		this.chain = mock(ResourceTransformerChain.class);
	...
	public void transformManifest() throws Exception {
		MockServerHttpRequest request = MockServerHttpRequest.get("/META-INF/test.appcache").build();
		MockServerWebExchange exchange = MockServerWebExchange.from(request);
		VersionResourceResolver versionResolver = new VersionResourceResolver();
		versionResolver.setStrategyMap(Collections.singletonMap("/**", new ContentVersionStrategy()));
		...
		assertThat("should rewrite resource links", content,
				Matchers.containsString("/META-INF/foo-e36d2e05253c6c7085a91522ce43a0b4.css"));
		assertThat("should rewrite resource links", content,
				Matchers.containsString("/META-INF/bar-11e16cf79faee7ac698c805cf28248d2.css"));
		assertThat("should rewrite resource links", content,
				Matchers.containsString("/META-INF/js/bar-bd508c62235b832d960298ca6c0b7645.js"));
