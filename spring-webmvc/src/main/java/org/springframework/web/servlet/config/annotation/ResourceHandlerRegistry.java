...
/**
 * Stores registrations of resource handlers for serving META-INF resources such as images, css files and others
 ...
 * for which the handler should be invoked to serve META-INF resources (e.g. {@code "/resources/**"}).
 *
 * <p>Then use additional methods on the returned {@link ResourceHandlerRegistration} to add one or more
 * locations from which to serve META-INF content from (e.g. {{@code "/"},
...
public class ResourceHandlerRegistry {
	 ...
	 * that also accepts the {@link UrlPathHelper} used for mapping requests to META-INF resources.
	 * @since 4.3.13
	 */
	public ResourceHandlerRegistry(ApplicationContext applicationContext, ServletContext servletContext,
	...
	/**
	 * Add a resource handler for serving META-INF resources based on the specified URL path patterns.
	 * The handler will be invoked for every incoming request that matches to one of the specified
	 * path patterns.
	 * <p>Patterns like {@code "/META-INF/**"} or {@code "/css/{filename:\\w+\\.css}"} are allowed.
